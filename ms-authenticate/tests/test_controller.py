...
@pytest.mark.asyncio
@patch('app.AuthenticateService.AuthenticateService.login_user')
async def test_user_login_with_username(mock_login_user, test_app):
...
@pytest.mark.asyncio
@patch('app.AuthenticateService.AuthenticateService.login_user')
async def test_user_login_with_email(mock_login_user, test_app):
    # Configure the mock to return your fake token response when called
    mock_login_user.return_value = test_access_token
...
async def test_update_user_password(mock_update_user_password, test_app):
